...
try:
    ...
    attachment_fields = config["attachment_fields"]  # For field extraction

    # Extract BERT config with fallback defaults
...
# Remove duplicates in ALL_SUB_REQUEST_TYPES while preserving order
...
try:
    ...
    if not os.path.exists(request_type_model_path):
        raise FileNotFoundError(f"Request type model directory not found: {request_type_model_path}")
    if not os.path.exists(sub_request_type_model_path):
        raise FileNotFoundError(f"Sub-request type model directory not found: {sub_request_type_model_path}")

    # Load tokenizer and models with local_files_only=True
    tokenizer = BertTokenizer.from_pretrained(
        request_type_model_path,
...
# Function to classify email using fine-tuned BERT models with rule-based fallback
def classify_email(text: str) -> Tuple[str, str]:
    ...
    if not text:
        logging.warning("Empty email text, defaulting to 'Unknown' classification.")
    ...
    try:
        ...
        with torch.no_grad():
            ...
            predicted_request_idx = torch.argmax(logits, dim=1).item()
            request_type = REQUEST_TYPES[predicted_request_idx]

        # Step 2: Classify Sub-Request Type (only if the request type has sub-types)
        ...
        if request_type in SUB_REQUEST_TYPES and SUB_REQUEST_TYPES[request_type]:
            ...
            # Special case: If the sub-request type list contains only the request type itself, set sub_request_type to "General"
            if sub_request_candidates == [request_type]:
                ...
                with torch.no_grad():
                    ...
                    logits = outputs.logits
                    predicted_sub_request_idx = torch.argmax(logits, dim=1).item()
                    ...
                    if global_sub_request in sub_request_candidates:
                        ...
                        logging.warning(f"Predicted sub-request type '{global_sub_request}' not valid for request type '{request_type}'. Falling back to rule-based classification.")

                        # Rule-based fallback: Search for sub-request type in the text
                        ...
                        for sub_type in sub_request_candidates:
                            ...
                            # If no sub-request type is found in the text, default to the first candidate
                            sub_request_type = sub_request_candidates[0]
                            logging.info(f"Defaulting to first sub-request type: {sub_request_type}")
    ...
    # Rule-based fallback for request_type if BERT fails
    if request_type not in REQUEST_TYPES:
        logging.info("Falling back to rule-based classification for request_type...")
        text_lower = text.lower()
...
def detect_primary_intent(text: str, attachment_text: str = "") -> str:
...
def store_classification(type_id: str, email_id: int, request_type: str, sub_request_type: str, fields: Dict[str, str], primary_intent: str) -> None:
    if request_type not in REQUEST_TYPES:
