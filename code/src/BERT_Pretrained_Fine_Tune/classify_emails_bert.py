...
try:
    ...
    # Extract BERT configuration (optional, with default)
    bert_config = config.get("bert", {})  # Use get() to avoid KeyError
    bert_max_length = bert_config.get("max_length_input", 512)  # Default to 512 if not specified
...
def extract_sub_request_from_text(text: str, sub_request_candidates: List[str]) -> Optional[str]:
    ...
    if match:
        ...
        if extracted_sub_request in sub_request_candidates:
            ...
            return extracted_sub_request
        else:
            logging.warning(f"Extracted sub-request type '{extracted_sub_request}' not in valid candidates: {sub_request_candidates}")
    else:
        logging.warning("Could not extract sub-request type from email text.")
    return None

# Function to classify email using fine-tuned BERT models
def classify_email(text: str) -> Tuple[str, Optional[str]]:
    ...
    try:
        ...
        if request_type in SUB_REQUEST_TYPES and SUB_REQUEST_TYPES[request_type]:
            ...
            # Special case: If the sub-request type list contains only the request type itself, set sub_request_type to None
            if sub_request_candidates == [request_type]:
                ...
                with torch.no_grad():
                    ...
                    if global_sub_request in sub_request_candidates:
                        ...
                        # Fallback: Extract sub-request type from email text
                        sub_request_type = extract_sub_request_from_text(text, sub_request_candidates)
                        if sub_request_type is None:
                            # If extraction fails, default to the first valid sub-request type
                            sub_request_type = sub_request_candidates[0]
                            logging.info(f"Defaulting to first sub-request type: {sub_request_type}")

        logging.info(f"Classification result - Request Type: {request_type}, Sub-Request Type: {sub_request_type if sub_request_type else 'None'}")
        return request_type, sub_request_type

    except Exception as e:
        logging.error(f"Error during BERT classification: {e}")
        # Default to the first request type with no sub-request type in case of error
        return REQUEST_TYPES[0], None
...
def detect_primary_intent(text: str) -> str:
    ...
    for request_type in REQUEST_TYPES:
        if request_type.upper() in text.upper():
            logging.info(f"Primary intent detected: {request_type}")
            return request_type
    logging.info("Primary intent not detected, defaulting to first request type")
...
def store_classification(type_id: str, email_id: int, request_type: str, sub_request_type: Optional[str], fields: Dict[str, str], primary_intent: str) -> None:
